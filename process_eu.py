...
def main():
    ...
    os.makedirs("pages_eu", exist_ok=True)

    if not os.path.exists("occupations_eu.json"):
        print("occupations_eu.json not found. Run scrape_eu.py first.")
        return
    ...
    total_json = len([f for f in os.listdir("raw_eu") if f.endswith(".json")] if os.path.exists("raw_eu") else [])
    total_md = len([f for f in os.listdir("pages_eu") if f.endswith(".md")])
