...
def main():
    ...
    with open("occupations_eu.json", "w", encoding="utf-8") as f:
        json.dump(occupations, f, ensure_ascii=False, indent=2)
    
    print(f"Saved {len(occupations)} ISCO-08 Level 4 groups to occupations_eu.json")
    ...
    cached = len([f for f in os.listdir("raw_eu") if f.endswith(".json")])
    print(f"\nDone. {cached}/{len(occupations)} JSON files cached in raw_eu/")
