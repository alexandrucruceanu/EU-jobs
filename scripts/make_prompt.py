...
def main():
    ...
    with open("data/json/occupations_eu.json") as f:
        occupations = json.load(f)

    with open("data/csv/occupations_eu.csv") as f:
        csv_rows = {row["slug"]: row for row in csv.DictReader(f)}

    # Fallback to scores.json if scores_eu.json doesn't exist yet
    scores_file = "data/json/scores_eu.json" if os.path.exists("data/json/scores_eu.json") else "data/json/scores.json"
