...
def main():
    if not os.path.exists("data/json/occupations_eu.json"):
        print("data/json/occupations_eu.json not found. Please run scrape_eu.py first.")
        return
    ...
    eurostat_data = {}
    if os.path.exists("data/json/eurostat_real.json"):
        with open("data/json/eurostat_real.json", "r") as f:
            eurostat_data = json.load(f)

    # Process all regions found in eurostat_real.json
    for region_label in eurostat_data.keys():
        output_csv = f"data/csv/occupations_{region_label.lower()}.csv"
